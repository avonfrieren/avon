# 1. Speedrun Tool's Additions
Fork or new Speedrun Tool mod in which you can :

##### 1.1 Practice sheet implementation
* Colored final time cp / il (doable)
* Condensed time map / image in game (doable)
* Complete implementation / synchronisation of google sheet in game (hardest ?)
...
##### 1.4 Show delta
For each room show the delta even if it's + something, not only the timer being gold but also the actual delta -X or +X
...
* Make Madeline disapear / invincible without killing the golden type berry's (doable ?)
